...
acc_odr_t acc_odr = acc_odr_norm_500;
gyro_odr_t gyro_odr = gyro_odr_norm_500;
...
// Averaged at startup while the device is stationary.
// Subtracted from every gyro reading to eliminate bias drift.
#define GYRO_CAL_SAMPLES  256
static float gyro_offset_x = 0.0f;
static float gyro_offset_y = 0.0f;
static float gyro_offset_z = 0.0f;

// ── Integrated orientation (degrees, relative to startup) ────────────
// Updated each QMI8658_Loop() call via trapezoidal integration.
// Yaw = rotation about Z (heading), Pitch = about X, Roll = about Y.
static float imu_yaw   = 0.0f;
static float imu_pitch = 0.0f;
static float imu_roll  = 0.0f;
static uint32_t imu_last_us = 0;     // micros() timestamp of previous update

// Dead-zone: ignore gyro readings below this threshold (deg/s)
// to prevent noise from accumulating into drift while stationary.
// Increase this if drift persists; decrease if small motions are missed.
#define GYRO_DEADZONE  0.5f

// ── Complementary filter ─────────────────────────────────────────────
// Blends gyro integration (fast, but drifts) with accelerometer tilt
// (noisy short-term, but zero drift long-term).
// Alpha close to 1.0 = trust gyro more (snappy, slow drift correction).
// Alpha close to 0.0 = trust accel more (stable, but sluggish).
// 0.98 is a standard starting point for ~100 Hz update rate.
...
void QMI8658_Init(void)
{
    ...
    setGyroODR(gyro_odr);                       
    setGyroLPF(LPF_MODE_0);                // tightest filter (2.66% of ODR)                
    switch (gyro_scale) {                  
    ...
    // ── Gyro zero-rate offset calibration ────────────────────────
    // Device must be stationary during startup.
    // Discard first 64 samples to let the LPF settle, then average.
    printf("[IMU] Calibrating gyro (keep device still)...\r\n");
    for (int i = 0; i < 64; i++) {
        getGyroscope();
    ...
    gyro_offset_x = sumX / GYRO_CAL_SAMPLES;
    gyro_offset_y = sumY / GYRO_CAL_SAMPLES;
    gyro_offset_z = sumZ / GYRO_CAL_SAMPLES;
    printf("[IMU] Gyro offsets: X=%.4f  Y=%.4f  Z=%.4f\r\n",
           gyro_offset_x, gyro_offset_y, gyro_offset_z);

    // Seed the integration timestamp
    imu_last_us = (uint32_t)micros();
}
...
void QMI8658_Loop(void)
{
  ...
  float dt = (float)(now_us - imu_last_us) * 1e-6f;  // seconds
  imu_last_us = now_us;

  // Reject outliers (first call, long stall, or timer wrap)
  if (dt <= 0.0f || dt > 0.1f) return;

  // ── Apply dead-zone to gyro ──────────────────────────────────
  float gx = (fabsf(Gyro.x) > GYRO_DEADZONE) ? Gyro.x : 0.0f;
  float gy = (fabsf(Gyro.y) > GYRO_DEADZONE) ? Gyro.y : 0.0f;
  float gz = (fabsf(Gyro.z) > GYRO_DEADZONE) ? Gyro.z : 0.0f;

  // ── Accelerometer-derived tilt (absolute reference) ──────────
  // atan2 gives the angle of the gravity vector projected onto
  // each tilt axis.  Only valid when the device isn't accelerating
  // hard, but the complementary filter handles that by weighting
  // the gyro more (alpha = 0.98).
  float acc_pitch = atan2f(Accel.x, sqrtf(Accel.y * Accel.y + Accel.z * Accel.z)) * (180.0f / M_PI);
  float acc_roll  = atan2f(Accel.y, sqrtf(Accel.x * Accel.x + Accel.z * Accel.z)) * (180.0f / M_PI);

  // ── Complementary filter for pitch & roll ────────────────────
  // Gyro integration for fast response, accel for drift correction.
  imu_pitch = COMP_FILTER_ALPHA * (imu_pitch + gx * dt) + (1.0f - COMP_FILTER_ALPHA) * acc_pitch;
  imu_roll  = COMP_FILTER_ALPHA * (imu_roll  + gy * dt) + (1.0f - COMP_FILTER_ALPHA) * acc_roll;

  // ── Yaw: pure gyro integration (no absolute reference) ───────
  // Without a magnetometer, dead-zone + calibration is all we have.
  imu_yaw += gz * dt;
...
float IMU_GetYaw(void)   { return imu_yaw; }
float IMU_GetPitch(void) { return imu_pitch; }
float IMU_GetRoll(void)  { return imu_roll; }

void IMU_ResetOrientation(void)
{
  imu_yaw   = 0.0f;
  imu_pitch = 0.0f;
  imu_roll  = 0.0f;
...
void setAccLPF(lpf_t lpf)
{
    ...
    ctrl5 &= ~QMI8658_ALPF_MASK;       // bitwise NOT (was logical NOT !)
...
void setGyroLPF(lpf_t lpf)
{
    ...
    {
    uint8_t ctrl5 = QMI8658_receive(QMI8658_CTRL5);
    ctrl5 &= ~QMI8658_GLPF_MASK;       // bitwise NOT (was logical NOT !)
...
void getGyroscope(void)
{
    ...
    Gyro.x = Gyro.x * gyroScales - gyro_offset_x;
    Gyro.y = Gyro.y * gyroScales - gyro_offset_y;
    Gyro.z = Gyro.z * gyroScales - gyro_offset_z;
}
