...
// Updated automatically by QMI8658_Loop().
// Yaw = heading (rotation about Z), Pitch = about X, Roll = about Y.
float IMU_GetYaw(void);
float IMU_GetPitch(void);
float IMU_GetRoll(void);

// Reset all angles back to zero (re-establish "forward" direction).
